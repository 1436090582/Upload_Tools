...
from tkinter import Tk, Label, Text, Scrollbar, Frame, DISABLED, NORMAL, END
...
from tkinter import BOTTOM, CENTER, StringVar
...
processed_files = set()
debug_text = None
is_collecting = False  # 初始为False，表示未在采集
...
def fixed_login():
    ...
    try:
        ...
        response = requests.post(LOGIN_URL, headers=headers, json={"UserLogin": 'administrator', "Password": 'hq123'})
        if response.status_code == 200 and response.json().get("retCode") == "1":
...
def login(username, password, output_widget=None):
    login_data = {"UserLogin": 'administrator', "Password": 'hq123'}
    headers = {"Content-Type": "application/json;charset=UTF-8"}
...
class LogUploadApp:
    def __init__(self, token):
        ...
        self.log_dir = None
        self.processed_files = load_processed_files()  # 加载已处理文件
        self.observer = None  # 将 observer 作为实例变量
        ...
        self.main_win.protocol("WM_DELETE_WINDOW", self.on_close)  # 添加关闭事件
        self.main_win.mainloop()

    def on_close(self):
        """处理程序关闭事件"""
        self.stop_collecting()  # 停止监控
        save_processed_files()  # 保存已处理文件
    ...
    def start_collecting(self):
        ...
        operation_name = self.work_order_var.get()
        process_code = self.process_var.get()
        line_code = self.line_var.get()
        ...
        debug_log(f"准备启动日志采集: 工单={operation_name}, 工序={process_code}, 线体={line_code}, 目录={self.log_dir}")

        try:
            # 扫描现有文件
            debug_log("调用 scan_directory 函数前...")
            self.scan_directory(self.token, operation_name, process_code, line_code)
            debug_log("调用 scan_directory 函数后...")

            # 初始化 Observer 并启动监控，设置 recursive=True 以监控所有子目录
            debug_log("初始化 Observer 并启动监控")
            self.observer = Observer()
            event_handler = LogFileHandler(self.token, operation_name, process_code)
            self.observer.schedule(event_handler, self.log_dir, recursive=True)  # 设置 recursive=True
            self.observer.start()
    ...
    def scan_directory(self, token, operation_name, process_code, line_code):
        debug_log(f"进入 scan_directory 函数: token={token}, 工单={operation_name}, 工序={process_code}, 线体={line_code}")
        if not self.log_dir:
            debug_log("日志目录未选择，无法扫描。")
            return

        debug_log(f"扫描目录: {self.log_dir}, 已处理文件: {processed_files}")
        ...
        for root, dirs, files in os.walk(self.log_dir):
            for filename in files:
                ...
                if file_path not in processed_files:
                    ...
                    self.process_log_file(token, file_path, operation_name, process_code)
                else:
                    debug_log(f"文件 {file_path} 已处理或不是文件，跳过")

    def process_log_file(self, token, file_path, operation_name, process_code):
        if file_path in self.processed_files:  # 检查文件是否已处理
            debug_log(f"文件已处理，跳过: {file_path}")
            return
        """处理单个日志文件，包括校验和上传"""
        debug_log(f"开始处理文件: {file_path}")
